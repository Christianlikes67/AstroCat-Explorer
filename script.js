...
async function loadPicture(date = "") {
    ...
    try {
        ...
        if (elements.image) {
            const mediaUrl = data.media_type === "image" ? (data.hdurl || data.url) : data.url;
        ...
        if (elements.pictureDate) elements.pictureDate.textContent = `📅 ${data.date}` + (data.copyright ? ` • © ${data.copyright}` : "");
        if (elements.status) elements.status.textContent = `NASA APOD loaded (${data.media_type})`;
